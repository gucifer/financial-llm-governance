...
Source: [`preprint.tex`](preprint.tex) — no `.bib`. Six figures live in [`figures/`](figures/) and are referenced via `\graphicspath{{figures/}}`. Compiles with any modern LaTeX.

**Upload both the `.tex` AND the `figures/` folder** (6 PNGs) — they are a package. On Overleaf, drag the whole `paper/` folder in, or zip `preprint.tex` + `figures/` together.
...
- [ ] Upload **source** (`preprint.tex` + the 6 PNGs in `figures/`) to arXiv, not the PDF — arXiv recompiles.
