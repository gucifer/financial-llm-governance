...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item \textbf{An honest, recall-floor-constrained FPR reduction with component attribution.} Under the temporal split, a cost-sensitive threshold-optimized tree ensemble reduces FPR by 94.7\% relative at recall $\geq 0.65$ --- reported with its full recall/AUC/calibration costs, as an operating point rather than a leaderboard claim, and with an ablation attributing the reduction to the threshold search rather than the anomaly blend.
...
\textbf{Hybrid unsupervised--supervised fusion.} Injecting unsupervised outlier scores into a supervised fraud classifier is an established design: Carcillo et al. (2021) combine unsupervised anomaly scores at multiple granularities with a supervised model on real card-fraud data. Our pipeline is an instance of this family; what we add is the observation that when the anomaly signal is blended at inference, the same blending must be applied inside each cross-validation fold during threshold search, or the calibrated threshold does not transfer to the blended score distribution (\S4).

\textbf{Threshold selection under imbalance and shift.} Decision-threshold tuning on cross-validated or out-of-bag score distributions is standard practice for imbalanced problems (Esposito et al., 2021; implemented in scikit-learn as \texttt{TunedThresholdClassifierCV}, Pedregosa et al., 2011), and minimizing one error rate subject to a floor on the other is the Neyman--Pearson classification paradigm, for which umbrella algorithms with statistical guarantees exist (Tong et al., 2018). The failure of a threshold calibrated on one score distribution to transfer to a shifted or transformed distribution is likewise a known phenomenon in the threshold-moving and calibration-under-drift literature. We do not claim these mechanisms as novel; our contribution is measuring what they cost (recall, AUC, calibration) on a temporally split AML benchmark, with a component ablation attributing the gains (\S5.2).

\textbf{Gap.} To our knowledge, no widely cited Elliptic study both isolates the temporal-versus-random leakage effect as a controlled experiment and targets FPR under an explicit recall floor with full cost accounting (recall, AUC, calibration) and a component ablation. That intersection is our focus.

\section{Data}

Elliptic (Weber et al., 2019) comprises 203,769 Bitcoin transactions over 49 time steps, each with 166 features (94 local, 72 aggregated). About 23\% of nodes are labeled (21\% licit, 2\% illicit); we use the labeled subset. Edges encode Bitcoin flows (234,355 edges; symmetrized to $\approx$469k directed message-passing entries) for the graph models. Our primary protocol is the temporal split (train steps 1--34, test 35--49). The test period (steps 35--49) contains 16,670 labeled transactions: 15,587 licit and 1,083 illicit. This dataset exhibits severe distributional shift central to our threshold analysis: 164 of 165 features (99.4\%) differ significantly between the training and test periods (two-sample test; see \S5.4), so a decision threshold fit on the training period does not transfer naively to the test period.

\section{Method}

\textbf{Baseline.} XGBoost reproducing the tabular Elliptic/Feedzai setup on the temporal test set; this is our FPR reference.

\textbf{Hybrid pipeline.} (1) \emph{Cost-sensitive boosting:} \texttt{scale\_pos\_weight} set to the training licit-to-illicit ratio ($\approx 7.63{:}1$). (2) \emph{Recall-floor threshold optimization:} rather than the default 0.5 threshold, we search the precision--recall curve under cross-validation for the threshold minimizing FPR subject to recall $\geq 0.65$; the selected operating threshold is 0.83. (3) \emph{Blended anomaly signal:} an isolation forest (Liu et al., 2008) trained on the licit class contributes an anomaly score at blend weight 0.15, applied \emph{inside} the CV folds during threshold search so the threshold is calibrated on the same blended score distribution it will be applied to. Calibrating on raw XGBoost probabilities and applying the threshold to blended scores selects a cut that is too high for the blended distribution and collapses recall. The ablation in \S5.2 attributes the FPR reduction to component (2); component (3) contributes recall stability at the selected operating point.
...
\textbf{Evaluation.} Primary metric: FPR at the recall floor; secondary: precision, illicit-class F1, AUC-ROC; plus calibration (Brier, expected calibration error). Tree-model metrics are five-run means (seeds 0--4); GNN metrics are three-run means (seeds 0--2); the split-comparison control (\S5.1) is a single 70/30 split realization, sufficient because XGBoost is deterministic given a fixed split. To quantify leakage we re-run the identical XGBoost configuration under a random 70/30 split.

\textbf{Explainability and audit.} SHAP values (Lundberg \& Lee, 2017) for flagged transactions are exported to newline-delimited JSON structured for FINRA Rule 4511 (books and records) and SEC Rule 17a-4. Figure~\ref{fig:shap} shows the global attribution profile of the hybrid model.
...
\subsection{The headline: temporal leakage inflates reported performance}
...
\subsection{FPR reduction at a recall floor}
...
Recall (illicit)& 0.7239 & $0.6726 \pm 0.0135$ & $-0.05$ \\
Precision       & 0.8981 & $0.9945 \pm 0.0001$ & $+0.10$ \\
F1 (illicit)    & 0.8016 & $0.8024 \pm 0.0097$ & $+0.001$ \\
...
Hybrid 95\% CI $= t(0.975, 4~\text{df}) \times \text{SD}/\sqrt{5} = 2.776 \times \text{SD}/\sqrt{5}$; metrics with CI $< 0.0001$ shown without interval.
\end{flushleft}
\end{table}

The hybrid cuts FPR by 94.7\% relative while holding recall above the 0.65 floor and lifting precision to 0.99. In absolute terms the hybrid produces a mean of 4 false positives versus the baseline's 89 on the 16,670-transaction test period (15,587 licit, 1,083 illicit). Symmetrically, the hybrid misses a mean of 55.6 additional illicit transactions (FN 354.6 vs.\ 299): about 7\% of the illicit transactions the baseline caught are no longer flagged --- the operational price of the FPR reduction. The main source of run-to-run variance is the cross-validated threshold search (different CV folds per seed), producing a recall SD of 0.0109 (95\% CI $\pm 0.0135$) --- modest variability that does not alter the directional result. The recall and AUC reductions are the transparent cost of moving the operating point toward precision.

\textbf{Component attribution.} An ablation (Table~\ref{tab:ablation}; per-run values in \texttt{results/ablation\_blend.json}) decomposes the pipeline. The FPR reduction is attributable to the recall-floor threshold search, not to the anomaly blend: with the blend disabled, the pipeline reaches the same FPR (0.0003; mean 3.8 vs.\ 4.0 false positives). Cost weighting alone \emph{raises} FPR (0.0192) by pushing the operating point toward recall; it matters only in combination with the threshold search. The blend's marginal contribution is recall at the fixed operating point: a mean $+0.012$ recall ($0.6602 \rightarrow 0.6726$; positive in all five paired seeds, but not significant at $\alpha = 0.05$, paired $t(4) \approx 2.5$) and threshold stability --- without the blend, test recall falls below the 0.65 floor in two of five runs (minimum 0.6371); with it, all five runs satisfy the floor (minimum 0.6565). The blend also costs AUC ($0.9325 \rightarrow 0.8933$), so programs that consume ranked scores rather than a fixed operating point should omit it.

\begin{table}[h]
\centering
\caption{Component ablation (temporal split, 5 seeds each).}
\label{tab:ablation}
\begin{tabular}{lcccc}
\toprule
...
\caption{False-positive rate (left) and illicit-class F1 (right) by model family under the temporal split. GAT's FPR (14.3\%) dwarfs the tree models; the FP-optimized hybrid is visually indistinguishable from zero. The two tree models tie on F1 ($\approx$0.80) while GCN and GAT trail.}
\label{fig:family}
\end{figure}

Vanilla GCN and GAT underperform both tree models on FPR and F1 (Figure~\ref{fig:family}). GAT's FPR (0.1434) is roughly $480\times$ the hybrid's (0.0003) --- more than two orders of magnitude --- driven by low precision (0.27) despite competitive recall, i.e., it flags far too much.
...
Threshold optimization is not free. \textbf{Table~\ref{tab:calibration}} shows the hybrid is \emph{less} calibrated than the baseline: expected calibration error rises from 0.190 to 0.291 and Brier score from 0.0215 to 0.0285. A compliance program that consumes raw probability scores (e.g., for risk tiering) should recalibrate the hybrid (e.g., Platt/isotonic) before relying on its scores as probabilities; the operating-point decision (flag/no-flag) is unaffected.
...
Cost-sensitive threshold optimization adds value proportional to the baseline FPR. On Elliptic (baseline FPR $= 0.57\%$), it reduces FPR by 94.7\% in relative terms. On PaySim (baseline FPR $\approx 0\%$), the mechanism is inert and pure XGBoost is the better choice. A practitioner should measure baseline FPR before applying the recall-floor pipeline; if baseline FPR is already near zero, the additional CV threshold search adds variance without benefit. The XGBoost tabular component generalizes to mobile-money data (F1 0.87 on PaySim vs.\ 0.80 on Elliptic), partially addressing the single-dataset limitation.
...
(a) \textbf{Single primary dataset}; external validity to production banking data is not fully established (Elliptic is Bitcoin-specific and partially anonymized). \S5.5 shows the tabular component generalizes to mobile-money data (PaySim), but the FP-reduction mechanism adds value only where baseline FPR is non-trivial --- consistent with its design. (b) \textbf{GNN baselines} were tuned (hidden size, epochs, learning rate; dropout at the library default 0.4; 3 runs) but still failed to achieve the recall floor ($\geq 0.65$) under threshold optimization; temporal GNNs (e.g., EvolveGCN) may perform better. (c) The \textbf{90--95\% industry figure} describes the share of \emph{alerts} that are false positives in rule-based production systems (an alert precision of 5--10\%) --- a different quantity from the statistical FPR measured here, so no numerical comparison between our $0.0057 \rightarrow 0.0003$ result and that figure is valid; it is illustrated and aligned with, not proven. (d) \textbf{Feature anonymization} limits semantic interpretability of SHAP outputs. (e) \textbf{Variance reporting:} five-run means reported with 95\% CI ($t$ distribution, 4 df) for the hybrid (Table~2); seeds 0--4. Baseline XGBoost is deterministic across seeds at 4 d.p.; hybrid recall SD = 0.0109 (95\% CI $\pm 0.0135$), driven by the cross-validated threshold search. GNN runs use seeds 0--2 (3 runs). Full per-run metrics are in \texttt{results/full\_results.json}. (f) The \textbf{recall floor (0.65)} is a demonstration value, below typical production AML detection targets. (g) \textbf{Batch-relative anomaly scoring:} the isolation-forest score is min-max normalized within the scored batch before blending, so each transaction's blended score depends on the batch it arrives with; streaming (single-transaction) deployment would require freezing the normalization constants from training-period licit scores. The reported operating points are therefore batch-evaluated, and we leave the frozen-constant variant to future work.
...
\textbf{Intellectual property disclosure.} The within-fold anomaly-blend threshold-calibration method described in Section~4 is the subject of U.S. Provisional Patent Application No.~64/105,503 (filed July 6, 2026) by the author. Code is released for research and reproducibility purposes.
...
Carcillo, F., Le Borgne, Y.-A., Caelen, O., Kessaci, Y., Obl\'{e}, F., \& Bontempi, G. (2021). Combining unsupervised and supervised learning in credit card fraud detection. \emph{Information Sciences}, 557, 317--331. \url{https://doi.org/10.1016/j.ins.2019.05.042}
...
Esposito, C., Landrum, G. A., Schneider, N., Stiefl, N., \& Riniker, S. (2021). GHOST: Adjusting the decision threshold to handle imbalanced data in machine learning. \emph{Journal of Chemical Information and Modeling}, 61(6), 2623--2640. \url{https://doi.org/10.1021/acs.jcim.1c00160}
...
Maganti, S. (2026). When graph structure becomes a liability: A critical re-evaluation of graph neural networks for Bitcoin fraud detection under temporal distribution shift. \emph{arXiv preprint arXiv:2604.19514}. \url{https://arxiv.org/abs/2604.19514}
...
Pedregosa, F., Varoquaux, G., Gramfort, A., Michel, V., Thirion, B., Grisel, O., Blondel, M., Prettenhofer, P., Weiss, R., Dubourg, V., Vanderplas, J., Passos, A., Cournapeau, D., Brucher, M., Perrot, M., \& Duchesnay, \'{E}. (2011). Scikit-learn: Machine learning in Python. \emph{Journal of Machine Learning Research}, 12, 2825--2830.

Tong, X., Feng, Y., \& Li, J. J. (2018). Neyman-Pearson classification algorithms and NP receiver operating characteristics. \emph{Science Advances}, 4(2), eaao1659. \url{https://doi.org/10.1126/sciadv.aao1659}
