...
Elliptic (Weber et al., 2019) comprises 203,769 Bitcoin transactions over 49 time steps, each with 166 features (94 local, 72 aggregated). About 21\% of nodes are labeled; we use the labeled subset. Edges encode Bitcoin flows ($\approx$470k undirected edges) for the graph models. Our primary protocol is the temporal split (train steps 1--34, test 35--49). The test period (steps 35--49) contains 16,670 labeled transactions: 15,587 licit and 1,083 illicit. This dataset exhibits severe distributional shift central to our threshold analysis: 164 of 165 features (99.4\%) differ significantly between the training and test periods (two-sample test; see \S5.4), so a decision threshold fit on the training period does not transfer naively to the test period.
...
The hybrid cuts FPR by 94.7\% relative while holding recall above the 0.65 floor and lifting precision to 0.99. In absolute terms the hybrid produces a mean of 4 false positives versus the baseline's 89 on the 16,670-transaction test period (15,587 licit, 1,083 illicit). The main source of run-to-run variance is the cross-validated threshold search (different CV folds per seed), producing a recall SD of 0.0109 (95\% CI $\pm 0.0095$) --- modest variability that does not alter the directional result. The recall and AUC reductions are the transparent cost of moving the operating point toward precision.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{operating_curve.png}
\caption{FPR--Recall operating curve (Elliptic, temporal split). The hybrid curve (blue) lies above and left of the baseline curve (red) --- higher recall at every FPR level --- a direct consequence of cost-sensitive weighting. The hybrid operating point (diamond, FPR=0.0003, Recall=0.67) sits in the upper-left corner. GNN default and tuned operating points are overlaid as triangular markers; tuned GNNs achieve lower FPR at the cost of recall far below the 0.65 floor (dashed).}
...
Cost-sensitive threshold optimization adds value proportional to the baseline FPR. On Elliptic (baseline FPR $= 0.57\%$), it reduces FPR by 94.7\%. On PaySim (baseline FPR $\approx 0\%$), the mechanism is inert and pure XGBoost is the better choice. A practitioner should measure baseline FPR before applying the recall-floor pipeline; if baseline FPR is already near zero, the additional CV threshold search adds variance without benefit. The XGBoost tabular component generalizes to mobile-money data (F1 0.87 on PaySim vs.\ 0.80 on Elliptic), partially addressing the single-dataset limitation.
...
(a) \textbf{Single primary dataset}; external validity to production banking data is not fully established (Elliptic is Bitcoin-specific and partially anonymized). \S5.5 shows the tabular component generalizes to mobile-money data (PaySim), but the FP-reduction mechanism adds value only where baseline FPR is non-trivial --- consistent with its design. (b) \textbf{GNN baselines} were tuned (hidden size, epochs, lr, dropout; 3 runs) but still failed to achieve the recall floor ($\geq 0.65$) under threshold optimization; temporal GNNs (e.g., EvolveGCN) may perform better. (c) The \textbf{90--95\% industry FPR figure} is illustrated and aligned with, not independently proven. (d) \textbf{Feature anonymization} limits semantic interpretability of SHAP outputs. (e) \textbf{Variance reporting:} five-run means with 95\% CI are reported for the hybrid (Table~2); seeds 0--4. Baseline XGBoost is deterministic across seeds at 4 d.p.; hybrid recall SD = 0.0109 (95\% CI $\pm 0.0095$), driven by the cross-validated threshold search. GNN runs use seeds 0--2 (3 runs). Full per-run metrics are in \texttt{results/full\_results.json}. (f) The \textbf{recall floor (0.65)} is a demonstration value, below typical production AML detection targets.
