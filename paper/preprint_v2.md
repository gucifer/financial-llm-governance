...
Benchmark results on the public Elliptic Bitcoin dataset (Weber et al., 2019) are frequently misrepresented by two evaluation habits: (i) random train/test splits that leak future data into training, and (ii) F1-optimized objectives that ignore operational false-positive cost. We make evaluation honesty the primary contribution. Under one controlled protocol, we show that random splitting inflates illicit-class F1 from 0.80 (temporal, honest) to 0.94 (random, leaky) and AUC to 0.996, reproducing the optimistic magnitudes in public Elliptic notebooks. Within that honest protocol we target the operationally relevant objective: minimizing false-positive rate (FPR) subject to a fixed recall floor, motivated by supervisory estimates of 90–95% FPR and ~USD 25.3 billion in annual U.S. AML compliance cost (Coelho et al., 2019). A hybrid pipeline — cost-sensitive XGBoost with cross-validated recall-floor threshold optimization and an isolation-forest anomaly blend — reduces FPR from 0.0057 to 0.0003 (−94.7% relative) at recall 0.67 and precision 0.99, with the recall, AUC, and calibration costs of threshold-shifting reported in full; vanilla GCN and GAT baselines under the same protocol confirm that trees outperform static graph models on this metric.
...
Elliptic (Weber et al., 2019) comprises 203,769 Bitcoin transactions over 49 time steps, each with 166 features (94 local, 72 aggregated). About 21% of nodes are labeled; we use the labeled subset. Edges encode Bitcoin flows (≈470k undirected edges) for the graph models. Our primary protocol is the temporal split (train steps 1–34, test 35–49). The test period (steps 35–49) contains 16,670 labeled transactions: 15,587 licit and 1,083 illicit. This dataset exhibits severe distributional shift central to our threshold analysis: 164 of 165 features (99.4%) differ significantly between the training and test periods (two-sample test; see §5.4), so a decision threshold fit on the training period does not transfer naively to the test period.
