...
**Explainability and audit.** For flagged transactions we compute SHAP values (Lundberg & Lee, 2017) and export per-transaction records to newline-delimited JSON, structured for FINRA Rule 4370 and SEC Rule 17a-4 record-retention expectations.
...
For regulatory AI, the results map to the U.S. Treasury Financial Services AI Risk Management Framework emphasis on measurable performance and to FinCEN's AML-modernization agenda; the SHAP-to-audit-record layer addresses explainability and recordkeeping expectations embedded in FINRA Rule 4370 and SEC Rule 17a-4.
